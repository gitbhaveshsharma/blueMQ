...
export default function WhatsAppPage() {
  ...
  async function handleSendTest() {
    ...
    if (digitsOnly.length < 7 || digitsOnly.length > 15) {
      toast.error(
        "Phone must be 7-15 digits in international format (e.g. 919876543210)",
      );
      return;
    }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50 px-5 py-3">
          ...
          <p className="text-xs text-blue-700">
            BlueMQ now uses Meta WhatsApp Cloud API only. Configure credentials
            per entity to isolate accounts safely.
          </p>
        </div>
        ...
        {sessions.length === 0 && !listLoading ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Add Meta API credentials for an entity to activate WhatsApp
              delivery.
            </p>
            <button
              onClick={openNewSession}
          ...
          <div className="space-y-3">
            {sessions.map((s) => {
              const info =
                STATUS_STYLES[s.status] || STATUS_STYLES.not_configured;
              return (
                <div
                  key={s.entity_id}
  ...
  const statusInfo =
    STATUS_STYLES[session?.status] || STATUS_STYLES.not_configured;
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Manage Meta WhatsApp credentials for this entity
        </p>
      </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400">
              Only last 6 characters are shown after save.
            </p>
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700">
      ...
      {session && (
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div
            className={`flex items-center justify-between border-b px-6 py-4 ${statusInfo.bg}`}
          >
          ...
          <div className="p-6 space-y-3">
            {session.phone_number ? (
              ...
              <div className="text-xs text-gray-500">
                Phone number will appear after successful sends.
              </div>
            )}
            ...
            {session.meta_business_account_id && (
              <p className="flex items-center gap-1.5 text-xs text-gray-500">
                <Building2 size={12} /> Business Account ID:{" "}
                <span className="font-medium">
                  {session.meta_business_account_id}
                </span>
              </p>
            ...
            {session.meta_api_key && (
              <p className="flex items-center gap-1.5 text-xs text-gray-500">
                <KeyRound size={12} /> Saved API Key:{" "}
                <span className="font-medium">{session.meta_api_key}</span>
              </p>
            )}
